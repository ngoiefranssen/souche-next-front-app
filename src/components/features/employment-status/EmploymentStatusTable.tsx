...
export const EmploymentStatusTable: React.FC<EmploymentStatusTableProps> = ({
  employmentStatuses,
  loading = false,
  pagination,
  onEdit,
  ...
  const actions: ActionButton<EmploymentStatus>[] = [];

  if (onEdit && hasPermission('employment-status:update')) {
    actions.push({
      label: 'Modifier',
  ...
  if (onDelete && hasPermission('employment-status:delete')) {
    actions.push({
      label: 'Supprimer',
      icon: <Trash2 className="w-4 h-4" />,
      onClick: onDelete,
      variant: 'danger',
      show: status => (status._count?.users ?? 0) === 0,
    });
  }
  ...
  return (
    <DataTable
      ...
      onSort={onSort}
      onFilter={onFilter}
      emptyMessage="Aucun statut d'emploi trouvé"
      className="mt-4"
    />
  );
};
