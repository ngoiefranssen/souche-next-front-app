  ...
  const loadData = useCallback(
    async (options?: { background?: boolean }) => {
      ...
      if (!Number.isFinite(roleId) || roleId <= 0) {
        showToast({
          message: 'Identifiant de rôle invalide',
          variant: 'error',
        });
      ...
      try {
        const [roleResponse, permissionsByCategoryResponse, assignedResponse] =
          await Promise.all([
    ...
    [locale, roleId, router, showToast]
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const assignTasks = changeSet.toAssign.map(permissionId => {
        return () => permissionsAPI.assign({ roleId, permissionId });
      });

      const revokeTasks = changeSet.toRevoke.map(permissionId => {
        return () => permissionsAPI.revoke({ roleId, permissionId });
      });
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
              Affecter Permissions au Rôle
            </h1>
            <p className="text-sm sm:text-base text-gray-600 mt-1">
              {role
                ? `Configurez les permissions pour le rôle \"${role.label}\".`
                : 'Chargement du rôle...'}
            </p>
          </div>
